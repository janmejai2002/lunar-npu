...
| **BUG-007** | 2026-09-10 02:15 | Critical | MicroRouter / Headless CI | GitHub Actions CI builds failed across Windows & Ubuntu matrix in `test_lunar_router.py` | Headless CI environments lack physical NPU model weights; fallback dense projection used non-deterministic `hash()` and positional matrix multiplication without semantic BoW or lexical priors | Autonomous Core Engine Lead | **VERIFIED_FIX** | Implemented deterministic OpenVINO BoW gather graph, MD5 tokenizer, and RouteLLM hybrid scoring; added `test_router_headless_fallback_simulation`; 26/26 tests passing |
| **BUG-008** | 2026-09-10 02:30 | Low | Test Harness / Environment | `pytest` CLI crashed on machines with broken third-party `pytest-httpbin` missing `brotlicffi` | Unmanaged global pytest plugins automatically hook into pytest session startup unless explicitly disabled | DevOps & Test Infrastructure Lead | **VERIFIED_FIX** | Configured `pyproject.toml` with `addopts = "-v -p no:httpbin ..."` and added `tests/test_lunar_cli.py`; 32/32 tests passing cleanly |
